...
export class CCTV {
    ...
    public init(hostname: string, username: string, password: string, port: number): any {
        this.cam = new Promise((resolve, reject) => {
            let retCCTVInformation: any = new onvif.Cam({ hostname, username, password, port }, (error: Error) => {
                if (error) {
                    reject(error);
                }
